...
// }
  ...
  List data = ["ekkll", "jinpoiwehnoi", "djbib"];
  late int i;
  List sum = [];
  calldata() {
    for (i = 0; i <= data.length;) return data[i];
  }
  ...
  createnoti() async {
    final response = await http.post(
      ...
      body: json.encode(<dynamic, dynamic>{
        "to":
            "egy6t6jeRc2edIzhHwnh_x:APA91bHM5luPcg6-jmytEhW83UoXYvAcGklMIvVgYV2QXCRVaxdBJ1o5aLjayQA_RWlvhtkk6JSUD28mXdM_dAcfhEd1AZgfhWazgUB1_gwBGPHtH2rcGN7q4gR5_pu8v4ygPr8g9Jg1",
        "notification": {
          "title": "Book Fair Appointment😊😊",
          "body": data.join("\n").toString(),

          //data.toString().split(",").join(","),
          //"Fair Start 19 January Book Now✋✋",
          "image":
